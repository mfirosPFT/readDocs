...
const sendMessage =  (data: object) => {
  ...
  // const res = await api.post(url.SEND_MESSAGE2, data);

  // console.log("res", api.create(url.SEND_MESSAGE, data));
  return api.create(url.SEND_MESSAGE, data);
};

const receiveMessage = (id: string | number) => {
  console.log("receiveMessage", api.update(url.RECEIVE_MESSAGE + "/" + id, { params: { id } }));
